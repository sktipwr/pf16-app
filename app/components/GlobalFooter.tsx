...
export default function GlobalFooter() {
  ...
  useEffect(() => {
    fetch("/api/counter/visits")
      ...
      .catch(() => {});
    fetch("/api/counter/attempts").catch(() => {});
  }, []);

  return (
    <>
      {/* Static footer — end of page, not fixed */}
      <div style={{ background: "linear-gradient(135deg, #0f1b2d, #1a2840)" }}>
        <div
          className="max-w-3xl mx-auto px-4 sm:px-6 py-2 flex items-center justify-between text-[10px]"
        ...
        >
          ...
          <span style={{ color: "rgba(255,255,255,0.4)" }}>
            {visitCount != null ? visitCount.toLocaleString("en-IN") : "—"} visitors
          </span>
        </div>
      </div>
